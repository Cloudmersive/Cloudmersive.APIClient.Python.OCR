...
> ReceiptRecognitionResult image_ocr_photo_recognize_receipt(image_file, language=language)
...
try:
    # Recognize a photo of a receipt, extract key business information
    api_response = api_instance.image_ocr_photo_recognize_receipt(image_file, language=language)
    pprint(api_response)
except ApiException as e:
