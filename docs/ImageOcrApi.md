...
> PhotoToWordsWithLocationResult image_ocr_photo_words_with_location(image_file, language=language, preprocessing=preprocessing)
...
try:
    # Convert a photo of a document or receipt into words with location
    api_response = api_instance.image_ocr_photo_words_with_location(image_file, language=language, preprocessing=preprocessing)
    pprint(api_response)
except ApiException as e:
