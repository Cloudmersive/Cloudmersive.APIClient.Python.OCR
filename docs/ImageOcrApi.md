...
> ImageToLinesWithLocationResult image_ocr_image_lines_with_location(image_file, language=language)
...
> ImageToWordsWithLocationResult image_ocr_image_words_with_location(image_file, language=language)
...
try:
    # Convert a scanned image into words with location
    api_response = api_instance.image_ocr_image_words_with_location(image_file, language=language)
    pprint(api_response)
except ApiException as e:
